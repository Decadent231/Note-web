...
- 文件库管理：文件上传（拖拽/选择）、下载、预览（图片/PDF/文本）、虚拟文件夹分类、搜索筛选、存储统计
- 工作台增强：8 项指标总览、本周效率报告（对比分析）、活动热力图（90 天 Calendar Heatmap）、知识增长曲线（30 天折线图）、优先待办列表
- 侧边栏分区：效率工具（工作台/待办/日历）、知识管理（笔记/知识库/文件库）、安全与系统（保险箱/回收站/日志/个人中心）
- 深浅主题切换与响应式布局
